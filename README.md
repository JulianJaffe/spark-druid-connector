...
The `PasswordProviderRegistry` provides support for deserializing dynamic configuration values via Druid
[PasswordProviders](https://druid.apache.org/docs/0.20.2/operations/password-provider.html).
...
provided as a serialized PasswordProvider that will be resolved when the metadata client is first instantiated. If
a  custom PasswordProvider is used, be sure to register the provider with the PasswordProviderRegistry before use.
...
|`reader.useDefaultValueForNull`|If true, use Druid's default values for null values. If false, explicitly use null for null values. See the [Druid configuration reference](https://druid.apache.org/docs/0.20.2/configuration/index.html#sql-compatible-null-handling) for more details|No|True|
...
[ingestion specs documentation](https://druid.apache.org/docs/0.20.2/ingestion/index.html#ingestion-specs) for more details.
...
|`writer.metrics`|The [metrics spec](https://druid.apache.org/docs/0.20.2/ingestion/index.html#metricsspec) used to define the metrics for the segments written to Druid. `fieldName` must match a column in the source dataframe|No|`[]`|
...
|`writer.segmentGranularity`|The chunking [granularity](https://druid.apache.org/docs/0.20.2/querying/granularities.html) of the Druid segments written (e.g. what granularity to partition the output segments by on disk)|No|`all`|
|`writer.queryGranularity`|The resolution [granularity](https://druid.apache.org/docs/0.20.2/querying/granularities.html) of rows _within_ the Druid segments written|No|`none`|
...
|`writer.useDefaultValueForNull`|If true, use Druid's default values for null values. If false, explicitly use null for null values. See the [Druid configuration reference](https://druid.apache.org/docs/0.20.2/configuration/index.html#sql-compatible-null-handling) for more details|No|True|
...
Users can also specify properties to be passed along to the [IndexSpec](https://druid.apache.org/docs/0.20.2/ingestion/index.html#indexspec). The
...
These configs generally shadow the [Connecting to S3 configuration](https://druid.apache.org/docs/0.20.2/development/extensions-core/s3.html#connecting-to-s3-configuration)
...
|`s3.accessKey`|The S3 access key. See [S3 authentication methods](https://druid.apache.org/docs/0.20.2/development/extensions-core/s3.html#s3-authentication-methods) for more details||
|`s3.secretKey`|The S3 secret key. See [S3 authentication methods](https://druid.apache.org/docs/0.20.2/development/extensions-core/s3.html#s3-authentication-methods) for more details||
|`s3.fileSessionCredentials`|The path to a properties file containing S3 session credentials. See [S3 authentication methods](https://druid.apache.org/docs/0.20.2/development/extensions-core/s3.html#s3-authentication-methods) for more details||
...
These configs shadow the [Google Cloud Storage Extension](https://druid.apache.org/docs/0.20.2/development/extensions-core/google.html) docs. The environment variable
