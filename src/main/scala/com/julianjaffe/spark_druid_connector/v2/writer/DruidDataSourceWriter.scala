...
import com.julianjaffe.spark_druid_connector.utils.SegmentRationalizer
...
class DruidDataSourceWriter(
  ...
  override def commit(writerCommitMessages: Array[WriterCommitMessage]): Unit = {
    ...
    metadataClient.publishSegments(rationalizedSegments.toList.asJava)
  }
  ...
  override def abort(writerCommitMessages: Array[WriterCommitMessage]): Unit = {
    ...
    segments.foreach { segmentStr =>
      ...
      try {
        ...
        case e: Exception =>
          logWarn(s"Unable to clean segment ${segment.getId.toString} with load spec " +
